...
- (void)imageURLStringGroup:(NSArray *)imageURLStringsGroup withColorValue:(NSArray *)colorValue
{
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    cell.backgroundColor=[self colorWithHexString:@"#F6F6F6"];
    ...
    NSString*colorStr = self.colorValue.count==self.imagePathsGroup.count? self.colorValue[itemIndex]:@"";
...
//        NSLog(@"588:%@",imagePath);
        if ([imagePath hasPrefix:@"http"]) {
            ...
            {
                colorValue=colorStr;
            }else if([colorStr isEqualToString:@""])
            {
        
            }
            else
            {
                colorValue = [NSString stringWithFormat:@"#%@",colorStr];
            }
            cell.backView.backgroundColor=[self colorWithHexString:colorValue];
        }
        else
        {
...
- (UIColor*)colorWithHex:(NSUInteger)hexColor alpha:(CGFloat)opacity {
...
- (UIColor*)colorWithHexString:(NSString *)hexStr alpha:(CGFloat)opacity {
    unsigned int hexint = 0;
    NSScanner *scanner = [NSScanner scannerWithString:hexStr];
    [scanner setCharactersToBeSkipped:[NSCharacterSet
                                       characterSetWithCharactersInString:@"#"]];
    [scanner scanHexInt:&hexint];
    return [self colorWithHex:hexint alpha:opacity];
}
