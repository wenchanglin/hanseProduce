...
#import "CenterAlterController1.h"
#import <Foundation/Foundation.h>
#import "AAPLCustomPresentationController.h"
@interface CenterAlterController1 ()<UIViewControllerTransitioningDelegate>
@property(nonatomic, strong)UILabel *titleLabel;
@property(nonatomic, strong)UILabel *contentLabel;
@property(nonatomic, strong)UIView *lineLayerView;
...
@implementation CenterAlterController1
...
-(void)showFromSourceController: (nonnull UIViewController *)source {
    if (source == nil) {
        source = [[[UIApplication sharedApplication]keyWindow]rootViewController];//UIApplication.topViewController;
    }
    AAPLCustomPresentationController *presentationController NS_VALID_UNTIL_END_OF_SCOPE;
    presentationController = [[AAPLCustomPresentationController alloc] initWithPresentedViewController:source presentingViewController:self];
    source.transitioningDelegate = presentationController;
    [self presentViewController:source animated:YES completion:NULL];
}
...
-(void)viewDidLayoutSubviews {
    ...
    if (self.message != nil || self.message.string.length > 0) {
        self.lineLayerView.top = self.contentLabel.bottom + space;
    } else {
        self.lineLayerView.top = self.contentLabel.bottom;
    }
    if (self.hasOkButton && self.hasCancelButton) {
        self.cancelButton.left = 15;
        self.cancelButton.top = self.lineLayerView.bottom + 14;
        ...
        self.okButton.top = self.lineLayerView.bottom + 14;
    }
}
...
-(void)configUI {
    ...
    if (self.hasOkButton || self.hasOkButton) {
        self.lineLayerView.width = vw;
        self.lineLayerView.height = 1;
        [self.view addSubview:self.lineLayerView];
        totalHeight += 1;
    }
    CGFloat bh = 0;
    CGFloat bw = 0;
    ...
    UIImage *img1 = [UIImage dd_roundImageWithColor:[UIColor colorWithHexString:@"#EEEEEE"]  size:self.cancelButton.size];//imageWithColor:size:self.cancelButton.size] imageByRoundCornerRadius:bh * 0.5];
    ...
    UIImage *img2 = [UIImage dd_roundImageWithColor:[UIColor clearColor] size:self.okButton.size];
...
-(UILabel *)titleLabel {
    if (_titleLabel == nil) {
        ...
        _titleLabel.textColor = [UIColor colorWithHexString:@"#F3F3F3"];
...
-(UIView *)lineLayerView {
    if (_lineLayerView == nil) {
        _lineLayerView = [[UIView alloc] init];
        _lineLayerView.backgroundColor = [UIColor colorWithHexString:@"#E1E1E1"];
    }
    return _lineLayerView;
}
-(UIButton *)cancelButton {
    if (_cancelButton == nil) {
        _cancelButton = [[UIButton alloc] initWithFrame:CGRectZero];
        [_cancelButton setTitleColor:[UIColor colorWithHexString:@"#F6F6F6"] forState:UIControlStateNormal];
        [_cancelButton setTitleColor:[UIColor colorWithHexString:@"#F6F6F6"] forState:UIControlStateHighlighted];
